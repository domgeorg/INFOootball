...
public class RecyclerViewHeadline extends RelativeLayout{
    ...
    public RecyclerViewHeadline(Context context){
        super(context);
    }

    public RecyclerViewHeadline(Context context,AttributeSet attrs){
        super(context,attrs);
    }

    public RecyclerViewHeadline(Context context,AttributeSet attrs,int defStyle){
        super(context,attrs,defStyle);
    }

    public final void attachTo(@NonNull final RecyclerView recycler){
        validate(recycler); this.recyclerView = RecyclerViewDelegate.with(recycler);
        this.layoutManager = LayoutManagerDelegate.with(recycler.getLayoutManager());
        isVertical = layoutManager.isVertical(); isAttachedToRecycler = true;
        ...
        recyclerView.setOnChildAttachListener(new RecyclerView.OnChildAttachStateChangeListener(){
            ...
            public void onChildViewDetachedFromWindow(View view){
                recycler.post(new Runnable(){
                    @Override
                    public void run(){
                        recyclerView.invalidateItemDecorations(); onScrollChanged();
                    }
                });
            }
        });
    ...
    public final void detach(){
        if(isAttachedToRecycler){
            isAttachedToRecycler = false; recyclerWantsTouch = false; recyclerView.reset();
            recyclerView = null; layoutManager = null;
        }
    }

    private void onScrollChanged(){
        hidden = recyclerView.hasItems() && ! layoutManager.isFirstRowVisible();
        RecyclerViewHeadline.super.setVisibility(hidden? INVISIBLE : intendedVisibility);
        if(! hidden){
            final int translation = calculateTranslation(); if(isVertical){
    ...
    public final void setVisibility(@Visibility int visibility){
        this.intendedVisibility = visibility; if(! hidden){
    ...
    protected final void onLayout(boolean changed,int l,int t,int r,int b){
        super.onLayout(changed,l,t,r,b); if(changed && isAttachedToRecycler){
            int verticalMargins = 0; int horizontalMargins = 0;
            ...
            recyclerView.onHeaderSizeChanged(getHeight() + verticalMargins,getWidth() +
                                                                           horizontalMargins);
            onScrollChanged();
        }
    }
    ...
    public boolean onInterceptTouchEvent(MotionEvent motionEvent){
        recyclerWantsTouch = isAttachedToRecycler && recyclerView.onInterceptTouchEvent
                                                                          (motionEvent);
        if(recyclerWantsTouch && motionEvent.getAction() == MotionEvent.ACTION_MOVE){
            downTranslation = calculateTranslation();
        } return recyclerWantsTouch || super.onInterceptTouchEvent(motionEvent);
    }
    ...
    public boolean onTouchEvent(@NonNull MotionEvent motionEvent){
        if(recyclerWantsTouch){
            ...
            MotionEvent recyclerEvent = MotionEvent.obtain(motionEvent.getDownTime(),motionEvent
                                                                                             .getEventTime(),motionEvent.getAction(),motionEvent.getX() - horizontalDiff,motionEvent.getY() - verticalDiff,motionEvent.getMetaState());
            recyclerView.onTouchEvent(recyclerEvent); return false;
        } return super.onTouchEvent(motionEvent);
    }

    private void validate(RecyclerView recyclerView){
        if(recyclerView.getLayoutManager() == null){
            throw new IllegalStateException("Attach RecyclerViewHeadline after setting " +
    ...
    private static class RecyclerViewDelegate{
        ...
        public final void onHeaderSizeChanged(int height,int width){
            if(decoration != null){
                decoration.setHeight(height); decoration.setWidth(width);
        ...
        private void invalidateItemDecorations(){
            if(! recyclerView.isComputingLayout()) recyclerView.invalidateItemDecorations();
        }
        ...
        public final void setHeaderDecoration(HeaderItemDecoration decoration){
            clearHeaderDecoration(); this.decoration = decoration;
            recyclerView.addItemDecoration(this.decoration,0);
        }

        public final void clearHeaderDecoration(){
            if(decoration != null){
                recyclerView.removeItemDecoration(decoration); decoration = null;
            }
        }

        public final void setOnScrollListener(RecyclerView.OnScrollListener onScrollListener){
            clearOnScrollListener(); this.onScrollListener = onScrollListener;
            recyclerView.addOnScrollListener(this.onScrollListener);
        }

        public final void clearOnScrollListener(){
            if(onScrollListener != null){
                recyclerView.removeOnScrollListener(onScrollListener); onScrollListener = null;
            }
        }

        public final void setOnChildAttachListener(RecyclerView.OnChildAttachStateChangeListener
                                                           onChildAttachListener){
            clearOnChildAttachListener(); this.onChildAttachListener = onChildAttachListener;
        ...
        public final void reset(){
            clearHeaderDecoration(); clearOnScrollListener(); clearOnChildAttachListener();
        }
    ...
    private static class LayoutManagerDelegate{
        ...
        private LayoutManagerDelegate(@NonNull RecyclerView.LayoutManager manager){
            ...
            if(managerClass == LinearLayoutManager.class){
                linear = (LinearLayoutManager)manager; grid = null; staggeredGrid = null;
            }else if(managerClass == GridLayoutManager.class){
                linear = null; grid = (GridLayoutManager)manager; staggeredGrid = null;
            }else
                throw new IllegalArgumentException("RecyclerViewHeadline supports only " +
        ...
        public final int getFirstRowSpan(){
            ...
            else if(grid != null) return grid.getSpanCount(); return 0;
        }

        public final boolean isFirstRowVisible(){
            if(linear != null) return linear.findFirstVisibleItemPosition() == 0;
            else if(grid != null) return grid.findFirstVisibleItemPosition() == 0; return false;
        }

        public final boolean isReversed(){
            if(linear != null) return linear.getReverseLayout();
            else if(grid != null) return grid.getReverseLayout(); return false;
        }

        public final boolean isVertical(){
            if(linear != null) return linear.getOrientation() == LinearLayoutManager.VERTICAL;
            else if(grid != null) return grid.getOrientation() == LinearLayoutManager.VERTICAL;
            return false;
        }
    }
    ...
    private class HeaderItemDecoration extends RecyclerView.ItemDecoration{
        ...
        public void getItemOffsets(Rect outRect,View view,RecyclerView parent,RecyclerView.State
            ...
            final boolean headerRelatedPosition = parent.getChildLayoutPosition(view) <
                                                  firstRowSpan;
            ...
            if(layoutManager.isReversed()){
                outRect.bottom = heightOffset; outRect.right = widthOffset;
            }else{
                outRect.top = heightOffset; outRect.left = widthOffset;
            }
        }
    }
