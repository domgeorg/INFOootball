...
public class LeagueViewHolder<T extends Country>
        extends BaseViewHolder<T>{


    @BindView(R.id.card_league_league_badge)
    ImageView leagueImageView;
    @BindView(R.id.card_league_league_name)
    TextView leagueNameTextView;
    @BindView(R.id.card_league_league_name_country)
    TextView leagueCountryTextView;

    private T league;
    private View view;

    public LeagueViewHolder(View view,Action1<T> onClick){
        super(view);
        this.view=view;
    ...
    @Override public void bind(T item){
        this.league=item;
        Picasso.with(view.getContext()).load(league.getStrBadge()).into(leagueImageView);
